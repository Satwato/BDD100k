...
st.set_page_config(page_title="Qualitative Model Evaluation", layout="wide")
...
def load_processed_data(data_path: Path):
    ...
    try:
        ...
        df["gt_box_id"] = df.apply(
            lambda row: f"{row['image_name']}_{row['x1']:.2f}_{row['y1']:.2f}", axis=1
        )
        return df
...
def create_evaluation_dataframe(predictions, df_gt, id2cat):
    ...
    df_gt[["is_detected", "is_misclassified", "matched_pred"]] = pd.DataFrame(
        detection_info.tolist(), index=df_gt.index
    )
...
def draw_on_image(image_path: Path, gt_box_to_highlight, matched_pred, id2cat):
    ...
    gx1, gy1, gx2, gy2 = (
        int(gt_box_to_highlight["x1"]),
        int(gt_box_to_highlight["y1"]),
        int(gt_box_to_highlight["x2"]),
        int(gt_box_to_highlight["y2"]),
    )
    ...
    cv2.putText(
        image, gt_text, (gx1, gy1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2
    )
    ...
    if matched_pred:
        ...
        if gt_label_name == pred_label_name:
            st.success(
                f"Correct Classification: Predicted '{pred_label_name}' with IoU: {iou:.2f}"
            )
        else:
            st.error(
                f"Misclassification: Predicted '{pred_label_name}' instead of '{gt_label_name}'. IoU: {iou:.2f}"
            )
    else:
        st.warning(
            "False Negative: No prediction was matched to this ground truth object."
        )
...
if page == "Qualitative Explorer":
    ...
    st.info(
        "Visually inspect model performance on specific slices of data. Use the filters to find interesting cases."
    )
    ...
    with st.sidebar.form(key="filter_form"):
        ...
        filter_scene = st.multiselect(
            "Scene", sorted(df_eval["scene"].dropna().unique()), default=None
        )
        filter_weather = st.multiselect(
            "Weather", sorted(df_eval["weather"].dropna().unique()), default=None
        )
        filter_timeofday = st.multiselect(
            "Time of Day", sorted(df_eval["timeofday"].dropna().unique()), default=None
        )
    ...
    if submitted:
        ...
        if filter_occluded != "Any":
            filtered_df = filtered_df[
                filtered_df["occluded"] == (filter_occluded == "Yes")
            ]
        if filter_truncated != "Any":
            filtered_df = filtered_df[
                filtered_df["truncated"] == (filter_truncated == "Yes")
            ]
        if min_difficulty > 0:
        ...
        if filter_detection_status == "Correctly Detected (TP)":
            filtered_df = filtered_df[
                (filtered_df["is_detected"] == True)
                & (filtered_df["is_misclassified"] == False)
            ]
        ...
        st.info(
            f"Found **{len(filtered_df)}** ground truth objects matching your criteria."
        )
    ...
    if "current_sample" in st.session_state:
        ...
        st.write(
            f"**Context:** {sample.get('weather', 'N/A')}, {sample.get('scene', 'N/A')}, {sample.get('timeofday', 'N/A')}"
        )
    ...
    st.info(
        "This page provides a quantitative breakdown of the model's most common failure modes."
    )

    valid_categories = df_eval["category"].dropna().unique()
    analysis_category = st.selectbox(
        "Select a Ground Truth Category to Analyze:", sorted(valid_categories)
    )
    ...
    if analysis_category:
        ...
        correct_count = len(
            df_cat[
                (df_cat["is_detected"] == True) & (df_cat["is_misclassified"] == False)
            ]
        )
        ...
        with col1:
            ...
            if not misclassified_df.empty:
                ...
                misclassified_df["pred_category_name"] = misclassified_df[
                    "matched_pred"
                ].apply(lambda x: id2cat.get(x["pred_label"]) if x else None)
                ...
                st.success(
                    f"No misclassification errors found for '{analysis_category}'."
                )
        ...
        with col2:
            ...
            if not missed_df.empty:
                st.write(
                    f"**Characteristics of the {len(missed_df)} missed '{analysis_category}' objects:**"
                )
                ...
                fig, ax = plt.subplots()
                missed_df["scene"].value_counts().plot(
