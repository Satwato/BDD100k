...
class CFG:
    ...
    TRAINED_MODEL_PATH = (
        "/bdd_files/train/checkpoints/rtdetr-bdd-best-epoch=06-val_loss=5.1101.ckpt"
    )
...
def visualize_predictions(config, model, data_module, num_images=4):
    ...
    color_map = {
        name: tuple(int(c * 255) for c in colors(i)[:3])
        for i, name in enumerate(data_module.class_names)
    }
    ...
    for i in range(min(num_images, config.BATCH_SIZE)):
        ...
        original_image = Image.open(
            os.path.join(config.VAL_IMG_DIR, img_info["file_name"])
        ).convert("RGB")
        ...
        img_pred = draw_boxes(
            original_image, pred_boxes, pred_labels, pred_scores, color_map
        )
        ...
        img_gt = draw_boxes(
            original_image, gt_boxes, gt_labels, [1.0] * len(gt_labels), color_map
        )
...
def get_preds(model, data_module):
    ...
    for batch in tqdm(val_loader, desc="Calculating mAP"):
        ...
        for i in range(len(batch["labels"])):
            img_info = data_module.val_dataset.coco.loadImgs(batch["image_id"][i])[0][
                "file_name"
            ]
            boxes, labels = ann_to_target(batch["orig_boxes"][i])
            targets.append({"image_name": img_info, "boxes": boxes, "labels": labels})
        preds = [
            {k: v.detach().cpu() if hasattr(v, "detach") else v for k, v in d.items()}
            for d in preds
        ]
...
def calculate_mAP(preds, targets, data_module):
    ...
    try:
        ...
        if isinstance(map_per_class, torch.Tensor):
            ...
            print(
                "Could not retrieve per-class AP. Check if 'class_metrics=True' is set."
            )
...
def stratified_metrics(image_to_context, all_targets, all_preds, id2label):
    ...
    for stratum in strata:
        ...
        for value in unique_values:
            ...
            image_names_in_stratum = image_to_context[
                image_to_context[stratum] == value
            ].index
            ...
            stratum_map_metric = torchmetrics.detection.MeanAveragePrecision(
                class_metrics=True
            )
            stratum_map_metric.update(stratified_preds, stratified_targets)
            stratum_map_results = stratum_map_metric.compute()
            print_mAP_results(
                stratum_map_results, id2label, title=f"mAP for {stratum} = {value}"
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        image_to_context = df_granular.drop_duplicates("image_name").set_index(
            "image_name"
        )
